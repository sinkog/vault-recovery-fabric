...
vault/
  ...
  vault-recovery-unseal (K8s auth role)
    ...
    Policy vault-unseal: read secret/data/<localPath> (derived from recovery.selfName)
    TTL: 5m, token_num_uses: 2

  vault-rekey (K8s auth role)
    Bound to: vault-recovery SA
    Policy vault-rekey: read+write secret/data/<localPath> + sys/rekey/*
    TTL: 15m, token_num_uses: 20

  vault-unseal (K8s auth role)
    Bound to: vault SA (postStart)
    Policy vault-unseal: read secret/data/<localPath>
    TTL: 1h

  localPath derivation:
    recovery.selfName empty  → secret/vault/unseal-keys  (lab/legacy)
    recovery.selfName=vault-a → secret/recovery/vault-a/unseal-keys  (mesh)

  Note: KV v2 CLI path (secret/<path>) differs from policy path (secret/data/<path>).
  Policies are generated by the vrf.localSecretPath / vrf.fallbackSecretPath helpers.
```
